...
BEGIN
	...
	WHERE 
	    ...
	    AND (
	        (E.forma_pago = 1 AND E.fecha_inicial <= ultimo_dia AND E.fecha_final >= primer_dia)
	        OR (E.forma_pago = 2 AND 
	            (MOD(EXTRACT(MONTH FROM AGE(primer_dia, E.fecha_inicial)), 2) = 0 OR 
	             (EXTRACT(MONTH FROM E.fecha_inicial) = EXTRACT(MONTH FROM ultimo_dia) AND E.fecha_inicial <= ultimo_dia AND E.fecha_final >= primer_dia)))
	        OR (E.forma_pago = 3 AND 
	            (MOD(EXTRACT(MONTH FROM AGE(primer_dia, E.fecha_inicial)), 3) = 0 OR 
	             (EXTRACT(MONTH FROM E.fecha_inicial) = EXTRACT(MONTH FROM ultimo_dia) AND E.fecha_inicial <= ultimo_dia AND E.fecha_final >= primer_dia)))
	        OR (E.forma_pago = 4 AND 
	            (MOD(EXTRACT(MONTH FROM AGE(primer_dia, E.fecha_inicial)), 6) = 0 OR 
	             (EXTRACT(MONTH FROM E.fecha_inicial) = EXTRACT(MONTH FROM ultimo_dia) AND E.fecha_inicial <= ultimo_dia AND E.fecha_final >= primer_dia)))
	        OR (E.forma_pago = 5 AND 
	            (MOD(EXTRACT(MONTH FROM AGE(primer_dia, E.fecha_inicial)), 12) = 0 OR 
	             (EXTRACT(MONTH FROM E.fecha_inicial) = EXTRACT(MONTH FROM ultimo_dia) AND E.fecha_inicial <= ultimo_dia AND E.fecha_final >= primer_dia)))
	    )
	GROUP BY 
