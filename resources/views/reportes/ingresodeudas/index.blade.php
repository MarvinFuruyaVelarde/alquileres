@extends('layouts.app')
@section('titulo','Reporte de Ingresos y Deudas')
@section('content')

@section('content')

<div class="pagetitle">
    <h1>Reporte de Ingresos y Deudas</h1>
    <div class="d-flex flex-row align-items-center justify-content-between">
        <nav>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('home') }}">Inicio</a></li>
            <li class="breadcrumb-item active">Reporte de Ingresos y Deudas</li>
        </ol>
        </nav>
    </div>
...
<section class="section">
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Reporte de Ingresos y Deudas</h5>
                            ...
                            <table class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">TOTAL GENERAL (FACTURADO)</th>
                                    </tr>
                                </thead>
                                <tbody id="dato">
...
<script>
    $(document).ready(function() {
        ...
        $('#regional, #aeropuerto, #fecha_inicial, #fecha_final').change(function() {
            actualizarReporte();
        });
        ...
        function actualizarReporte() {
            ...
            if (regional || aeropuerto || (fechaInicial && fechaFinal)) {
                $.ajax({
                    url: '{{ url("reporteingresodeudas/obtieneReporte/") }}',
                    method: 'get',
                    data: {'regional': regional, 'aeropuerto': aeropuerto, 'fechaInicial': fechaInicial, 'fechaFinal': fechaFinal},
        ...
        function mostrarConsulta(data) {
            ...
            if (data.length > 0) {
                data.forEach(function(item) {
                    grillaHtml += `
                        <tr>    
                            <td class="text-center col-3">${item.cod_regional || ''}</td>
                            <td class="text-center col-3">${item.cod_aeropuerto || ''}</td>
                            <td class="text-center col-3">${item.total_pagado || ''}</td>
                            <td class="text-center col-3">${item.total_deuda || ''}</td>
                            <td class="text-center col-3">${item.total_facturado || ''}</td>                         
                        </tr>
                    `;
                });
            } else {
    ...
    function generarReporte(tipo) {
        ...
        let url = tipo === 'pdf' 
            ? "{{ route('reporteingresodeudas.show') }}" 
            : "{{ route('reporteingresodeudas.export') }}";

        url += `?regional=${regional}&aeropuerto=${aeropuerto}&fechaInicial=${fechaInicial}&fechaFinal=${fechaFinal}`;
