                        ...
                        @foreach($expensas as $expensa)
                            ...
                            <tr>
                                ...
                                <td class="d-flex justify-content-center" >
                                    @can('expensas.edit')
                                        <a href="{{route('expensas.edit',$expensa)}}" class="btn btn-warning" title="Modificar Datos"><i class="bi bi-pencil-square"></i></a>
                                    @endcan
                                    {{--@can('expensas.destroy')
                                        <a href="{{route('expensas.destroy',$expensa->id)}}" class="btn btn-danger" title="Eliminar Registro" onclick="return confirm('¿Está seguro que desea eliminar la EXPENSA?');" ><i class="bi bi-trash"></i></a>
                                    @endcan--}}
                                    @can('expensas.destroy')
                                        <form action="{{ route('expensas.destroy', $expensa->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger" title="Eliminar Registro" onclick="return confirm('¿Está seguro que desea eliminar la EXPENSA?');"><i class="bi bi-trash"></i>
                                            </button>
                                        </form>
                                    @endcan
                                </td>
