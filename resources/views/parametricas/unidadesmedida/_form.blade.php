<p>
    ...
    Al momento de registrar/editar una nueva unidad de medida</p>
<div class="row mb-1">
    <label for="descripcion" class="col-md-4 col-form-label text-right">Descripcion: <span class="text-danger">(*)</span></label>
    <div class="col-md-6">
        <input id="descripcion" type="text" class="form-control {{ $errors->has('descripcion') ? ' error' : '' }}" name="descripcion" value="{{ old('descripcion',$unidadmedida->descripcion) }}" autofocus onkeyup="this.value = this.value.toUpperCase();" autocomplete="off" data-input-id="descripcion" data-validate="length" data-min-length="2" data-max-length="50">
        ...
        <select id="estado" class="form-control{{ $errors->has('estado') ? ' error' : '' }}" name="estado">
            ...
            @foreach($estados as $estado)
                <option value="{{ $estado->id }}" 
                    {{ old('estado', $unidadmedida->estado) == $estado->id ? 'selected' : '' }}>
                    {{ $estado->descripcion }}
                </option>
            @endforeach
        </select>
...
<div class="row mt-2">
    <div class="text-center">
        ...
        <a href="{{ route('unidadesmedida.index') }}" class="btn btn-warning">Cancelar</a>
    </div>
</div>
